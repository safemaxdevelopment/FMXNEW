...
#define PULSE_LIMIT_COUNT 170

// Constants
#define VEHICLE_SPEED_LIMIT  170 //Same as Pulse count
...
void main(void) {
    ...
    // DAC Settings
    TRISC = 0x00;  // Set all RC pins as output. This is for DAC mainly
    ANSEL = 0x00;
    LCDCON = 0x03;
    LCDCON = 0x00;
    LCDSE0 = 0x00;
    ...
    while (1) {
       ...
       if (timer_flag) 
           ...
           if (pulse_copy_count > 2) {
            ...
            if (pulse_copy_count > speed_limit) //speed_limit
            {
                 PORTAbits.RA2 = 1;
                 PORTBbits.RB4 = 0;
                 PORTBbits.RB3 = 1;
